...
var λ = function λ(url, filename, folder) {
    return new Promise(function (fulfill, reject) {
        ...
        setTimeout( function () {
            ...
            $.get('/_/' + url, null, null, 'text').then(function (data) {
            ...
            function (_, err) {
                ლ_ಠ益ಠ_ლ('failed to load /_/' + url + ' (' + filename + ')', err)
...
var define_ = window.define // fix JSZip not exporting properly
window.define = null
Promise.all([
    ...
    $.getJSON('/_/playlists').then(function (data) {
        ...
        Promise.all(promises).then(function () { // finished loading all resources
            // compile the zip and download it
            var date = /[^T]+/.exec(new Date().toISOString())[0]
            ...
            setTimeout(function () {
                $prog.fadeOut(function () { $prog.remove() })
            }, 5000)
        })
    }, function (_, err) {
